...
interface LeftFileTreeProps {
  ...
  handleUploadFileRequest: (folderId?: string) => void
  handleAddLinkRequest: (folderId?: string) => void
  handleImportCurriculumRequest: (folderId?: string) => void
...
const LeftFileTreeComponent: React.FC<LeftFileTreeProps> = ({
    ...
    return (
      <div key={folderId} className="space-y-1">
        ...
        >
          ...
          {/* Folder CRUD Actions */}
          {!isUncategorized && (
            <div className="relative flex items-center gap-1 opacity-0 group-hover:opacity-100 has-[[data-state=open]]:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
              <button
                type="button"
              ...
              <DropdownMenu>
                ...
                <DropdownMenuPortal>
                  <DropdownMenuContent align="end" sideOffset={4} className="w-48 rounded-2xl border border-slate-200 bg-white/95 p-1.5 shadow-2xl backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/95 z-50">
                    <DropdownMenuItem onSelect={() => handleUploadFileRequest(folderId)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                      <UploadCloud className="h-3.5 w-3.5 text-cyan-500" />
                      Dosya Yukle
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => handleAddLinkRequest(folderId)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                      <LinkIcon className="h-3.5 w-3.5 text-violet-500" />
                      Link Ekle
                    </DropdownMenuItem>
                    <DropdownMenuItem onSelect={() => handleImportCurriculumRequest(folderId)} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
  ...
  const renderResourceNode = (res: EducationResource) => {
    ...
    return (
      <div key={res.id} className="space-y-1">
        ...
        <div
          onClick={() => {
            handleTreeNodeClick('file', String(res.id))
        ...
        >
          ...
          <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity" onClick={e => e.stopPropagation()}>
            <button
              ...
              className="p-0.5 rounded text-slate-400 hover:text-slate-700 dark:hover:text-zinc-200 transition-colors"
              title="Yeniden Adlandır"
            >
              <Pencil className="h-3 w-3" />
            </button>
            <button
              ...
              className="p-0.5 rounded text-slate-400 hover:text-red-500 dark:hover:text-red-400 transition-colors"
              title="Dosyayı Sil"
            >
      ...
      <div className="space-y-1">
        ...
        >
          ...
          <button
            ...
            className="rounded-md p-1 text-slate-400 opacity-0 transition-all hover:text-slate-700 group-hover:opacity-100 dark:hover:text-zinc-200"
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50/50 dark:bg-zinc-950/20 rounded-2xl border border-slate-200 dark:border-zinc-800/60 p-4 space-y-4 overflow-visible">
      <div className="relative flex items-center justify-between shrink-0 border-b border-slate-200 dark:border-zinc-800 pb-2">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <button
              type="button"
              className="inline-flex h-7 w-7 items-center justify-center rounded-lg text-slate-400 transition-colors hover:text-slate-700 dark:hover:text-zinc-200"
          ...
          <DropdownMenuPortal>
            <DropdownMenuContent align="end" sideOffset={4} className="w-48 rounded-2xl border border-slate-200 bg-white/95 p-1.5 shadow-2xl backdrop-blur-xl dark:border-zinc-800 dark:bg-zinc-950/95 z-50">
              <DropdownMenuItem onSelect={handleCreateFolderRequest} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                <Folder className="h-3.5 w-3.5 text-amber-500" />
                Yeni Klasor
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => handleUploadFileRequest()} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                <UploadCloud className="h-3.5 w-3.5 text-cyan-500" />
                Dosya Yukle
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => handleAddLinkRequest()} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                <LinkIcon className="h-3.5 w-3.5 text-violet-500" />
                Link Ekle
              </DropdownMenuItem>
              <DropdownMenuItem onSelect={() => handleImportCurriculumRequest()} className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-xs font-bold text-slate-800 hover:bg-cyan-500/10 hover:text-cyan-700 dark:text-zinc-200 dark:hover:text-cyan-300 cursor-pointer">
                <ClipboardList className="h-3.5 w-3.5 text-emerald-500" />
                Mufredat Yapistir
              </DropdownMenuItem>
            </DropdownMenuContent>
